...
const htmlWebpackPlugin = require('html-webpack-plugin'); // 这个插件是用来生成html文件的
...
module.exports = {
    ...
    entry: {
        app: './src/main.js',
        vueAll:  ['vue', 'vuex'], // key值与CommonsChunkPlugin的name数组里面的一样
        axios: ['axios'] // key值与CommonsChunkPlugin的name数组里面的一样
    },
    ...
    output: {
        ...
        filename: isProduction ? 'js/[name].[chunkhash].js' : 'js/[name].js', // 路径相对于output.path（devServer不能使用chunkhash，所以判断一下）
        chunkFilename: 'js/common-[id].js', // 输出 chuck文件名
    ...
    module: {
        rules: [
            ...
            },
            {
                test: /\.js$/,
                loader: 'babel-loader',
                exclude: path.join(__dirname, './node_modules/'), // 不转换node_modules文件夹里面的js
            },
            {
                test: /\.(png|jpe?g|gif|svg)(\?.*)?$/,
                loader: 'url-loader',
                options: {
                    limit: 10000, // 小于1KB时，使用base64方式插入到css里面
    ...
    plugins: [
        ...
        new webpack.optimize.CommonsChunkPlugin({
            name: ['common', 'vueAll', 'axios'],
            minChunks: 2  //公共模块最小被引用的次数
        }),

        // css-loader提取后的统一输出路径
